...
def save_per_member(prod_id, now, member_stocks):
    folder_name = str(prod_id)
    excel_path = os.path.join(folder_name, f"{prod_id}.xlsx")
    ...
    for member_name, stocks in member_stocks.items():
        ...
        data = {
            ...
            "TotalStocks": [stocks],
            "UnitSales": [unit_sales if isinstance(unit_sales, int) else ""],
        }
        ...
        if os.path.exists(excel_path):
            with pd.ExcelWriter(excel_path, engine="openpyxl", mode='a', if_sheet_exists='overlay') as writer:
                try:
                    existing_df = pd.read_excel(excel_path, sheet_name=member_name)
                    if not existing_df.empty and unit_sales == 0:
                        existing_df.iloc[-1, existing_df.columns.get_loc("Time")] = now
                        existing_df.to_excel(writer, sheet_name=member_name, index=False)
                    else:
                        updated_df = pd.concat([existing_df, df], ignore_index=True)
                        updated_df.to_excel(writer, sheet_name=member_name, index=False)
                except ValueError:
                    # Sheet 不存在，直接写入
                    df.to_excel(writer, sheet_name=member_name, index=False)
        else:
