...
def save_per_member(prod_id, now, month_sales, sold_num, member_stocks):
    ...
    unit_sales_map = {}  # 记录 member -> (stocks, unit_sales)

    # === 1. 写入 CSV，同时记录旧 stocks 给 Excel 用 ===
    for member_name, stocks in member_stocks.items():
        ...
        if last_stocks is not None:
            ...
            unit_sales = ""
        unit_sales_map[member_name] = (stocks, unit_sales)
    ...
    # === 2. 写入 Excel：只记录变化时间点 ===
    for member_name, (stocks, unit_sales) in unit_sales_map.items():
        ...
        if os.path.exists(excel_path):
            with pd.ExcelWriter(excel_path, engine="openpyxl", mode='a', if_sheet_exists='overlay') as writer:
                try:
                    ...
                    if not existing_df.empty and existing_df.iloc[-1]["TotalStocks"] == stocks:
                        existing_df.at[existing_df.index[-1], "Time"] = now
